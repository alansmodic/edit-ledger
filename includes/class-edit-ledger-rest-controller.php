...
class EditLedgerRestController extends \WP_REST_Controller
{
    /**
     * Constructor.
     */
    ...
    public function registerRoutes()
    {
        ...
        register_rest_route(
            ...
            array(
                ...
                'callback'            => array( $this, 'getPostRevisions' ),
                'permission_callback' => array(
                    $this,
                    'getPostRevisionsPermissionsCheck',
                ),
                'args'                => array(
                    'post_id'  => array(
                        'required'          => true,
        ...
        register_rest_route(
            ...
            array(
                'methods'             => \WP_REST_Server::READABLE,
                'callback'            => array( $this, 'getRevisionDiff' ),
                'permission_callback' => array(
                    $this,
        ...
        register_rest_route(
            ...
            array(
                ...
                'permission_callback' => array(
                    $this,
                    'restoreRevisionPermissionsCheck',
                ),
                'args'                => array(
                    'revision_id' => array(
                        'required'          => true,
        ...
        register_rest_route(
            ...
            array(
                'methods'             => \WP_REST_Server::READABLE,
                'callback'            => array( $this, 'getRecentRevisions' ),
                'permission_callback' => array(
                    $this,
                    'getRecentPermissionsCheck',
                ),
                'args'                => array(
                    'per_page' => array(
                        'default'           => 20,
     ...
     */
    public function getPostRevisions($request)
    {
        ...
        foreach ($revision_array as $index => $revision) {
            ...
            $previous = isset($revision_array[ $index + 1 ])
                ...
                : $post;
            $data[]   = $this->formatRevision($revision, $previous);
        }
     ...
     * @return \WP_REST_Response|\WP_Error
     */
    public function getRevisionDiff($request)
    {
        ...
        if ($compare_to) {
            ...
            if (! $compare_post) {
                return new \WP_Error(
                    'not_found',
                    __('Comparison revision not found.', 'edit-ledger'),
                    array( 'status' => 404 )
                );
            }
        } else {
            // Find the previous revision.
            $compare_post = $this->getPreviousRevision($revision);
            if (! $compare_post) {
        ...
        $diff_generator = new EditLedgerDiffGenerator();

        // Clean content for readable diffs.
        $from_content = $this->stripHtmlForDiff($compare_post->post_content);
        $to_content   = $this->stripHtmlForDiff($revision->post_content);
        $from_excerpt = $this->stripHtmlForDiff($compare_post->post_excerpt);
        $to_excerpt   = $this->stripHtmlForDiff($revision->post_excerpt);

        // Extract media changes (images, videos, embeds).
        $media_changes = $this->getMediaChanges(
        ...
        $comparison = array(
            'from' => array(
                ...
                'date_relative' => human_time_diff(
                    strtotime($compare_post->post_modified_gmt),
                    time()
                ),
                'author'        => array(
                    'name'   => $compare_author
                        ? $compare_author->display_name
                        : __('Unknown', 'edit-ledger'),
                    'avatar' => get_avatar_url(
                        $compare_post->post_author,
                        array( 'size' => 48 )
                    ),
                ),
            ...
            'to'   => array(
                ...
                'date_relative' => human_time_diff(
                    strtotime($revision->post_modified_gmt),
                    time()
                ),
                'author'        => array(
                    'name'   => $revision_author
                        ? $revision_author->display_name
                        : __('Unknown', 'edit-ledger'),
                    'avatar' => get_avatar_url(
     ...
     * @return \WP_REST_Response|\WP_Error
     */
    public function getRecentRevisions($request)
    {
        global $wpdb;
        ...
        $query = $wpdb->prepare(
            "SELECT r.*, p.post_title as parent_title,
        ...
        foreach ($revisions as $revision) {
            $data[] = $this->formatRevisionForAdmin($revision);
        }
     ...
     * @return array
     */
    private function formatRevision($revision, $compare)
    {
     ...
     * @return array
     */
    private function formatRevisionForAdmin($revision)
    {
        $author      = get_userdata($revision->post_author);
    ...
    private function stripHtmlForDiff($content)
    {
        ...
        $content = preg_replace(
            '/<\/(p|div|h[1-6]|li|blockquote|pre)>/i',
            "\n",
            $content
        );
    ...
    private function convertMediaToPlaceholders($content)
    {
        ...
        $content = preg_replace_callback(
            ...
            function ($matches) {
                ...
                if (preg_match('/src=["\']([^"\']*)["\']/', $img, $src_match)) {
                    $filename = basename(
                        parse_url($src_match[1], PHP_URL_PATH)
                    );
                    if (! empty($filename)) {
        ...
        $content = preg_replace_callback(
            ...
            function ($matches) {
                ...
                if (
                    strpos($figure, 'twitter') !== false
                    || strpos($figure, 'x.com') !== false
                ) {
                    return '[Twitter/X Embed]';
                }
        ...
        $content = preg_replace_callback(
            ...
            function ($matches) {
                ...
                if (preg_match('/href=["\']([^"\']*)["\']/', $link, $href_match)) {
                    $filename = basename(
                        parse_url($href_match[1], PHP_URL_PATH)
                    );
                    if (! empty($filename)) {
     ...
     */
    private function getMediaChanges($from_content, $to_content)
    {
        $from_media = $this->extractMedia($from_content);
        $to_media   = $this->extractMedia($to_content);
    ...
    private function extractMedia($content)
    {
        ...
        // YouTube.
        $youtube_pattern = '/(?:youtube\.com\/(?:embed\/|watch\?v=)'
            . '|youtu\.be\/)([a-zA-Z0-9_-]+)/';
        if (preg_match_all($youtube_pattern, $content, $matches)) {
     ...
     */
    public function getPostRevisionsPermissionsCheck($request)
    {
        $post_id = $request->get_param('post_id');
        if (! current_user_can('edit_post', $post_id)) {
            return new \WP_Error(
                'forbidden',
                __(
                    'You do not have permission to view revisions for this post.',
                    'edit-ledger'
                ),
                array( 'status' => 403 )
            );
     ...
     * @return bool|\WP_Error
     */
    public function getRevisionPermissionsCheck($request)
    {
        $revision_id = $request->get_param('revision_id');
        $revision    = get_post($revision_id);
        ...
        if (! current_user_can('edit_post', $revision->post_parent)) {
            return new \WP_Error(
                'forbidden',
                __(
                    'You do not have permission to view this revision.',
                    'edit-ledger'
                ),
                array( 'status' => 403 )
            );
     ...
     * @return bool|\WP_Error
     */
    public function getRecentPermissionsCheck($request)
    {
        if (! current_user_can('edit_others_posts')) {
            return new \WP_Error(
                'forbidden',
                __(
                    'You do not have permission to view all revisions.',
                    'edit-ledger'
                ),
                array( 'status' => 403 )
            );
    ...
    public function restoreRevision($request)
    {
        ...
        if (! $restored) {
            return new \WP_Error(
                'restore_failed',
                __('Failed to restore revision.', 'edit-ledger'),
                array( 'status' => 500 )
            );
        }
     ...
     * @return bool|\WP_Error
     */
    public function restoreRevisionPermissionsCheck($request)
    {
        $revision_id = $request->get_param('revision_id');
        $revision    = get_post($revision_id);
        ...
        if (! current_user_can('edit_post', $revision->post_parent)) {
            return new \WP_Error(
                'forbidden',
                __(
                    'You do not have permission to restore this revision.',
                    'edit-ledger'
                ),
                array( 'status' => 403 )
            );
