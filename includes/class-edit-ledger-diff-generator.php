...
class EditLedgerDiffGenerator
{
    ...
    public function generate($from, $to)
    {
        ...
        $diff = $this->computeWordDiff($from, $to);
     ...
     */
    private function computeWordDiff($from, $to)
    {
        $from_words = $this->tokenize($from);
        $to_words   = $this->tokenize($to);

        $diff = $this->diffArrays($from_words, $to_words);
     ...
     */
    private function diffArrays($from, $to)
    {
        $from_len = count($from);
        $to_len   = count($to);
     ...
     */
    private function renderDiff($diff)
    {
        $html          = '';
        $current_type  = null;
        $current_text  = '';
        ...
        foreach ($diff as $op) {
            ...
            if ($type === $current_type) {
                ...
                $html        .= $this->wrapSegment($current_type, $current_text);
                $current_type = $type;
                $current_text = $text;
            }
        }
     ...
     * @return string HTML output.
     */
    private function wrapSegment($type, $text)
    {
        if (empty($text)) {
            return '';
        }
